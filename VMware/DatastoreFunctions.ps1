...
.EXAMPLE Get-Datastore Datastore | Unmount-Datastore
.EXAMPLE Get-Datastore Datastore | Get-DatastoreMountInfo | Sort Datastore, VMHost | FT -AutoSize
.EXAMPLE Get-Datastore Datastore | Mount-Datastore
.EXAMPLE Get-Datastore Datastore | Get-DatastoreMountInfo | Sort Datastore, VMHost | FT -AutoSize
.EXAMPLE Get-Datastore Datastore | Detach-Datastore
.EXAMPLE Get-Datastore Datastore | Get-DatastoreMountInfo | Sort Datastore, VMHost | FT -AutoSize
.EXAMPLE Get-Datastore Datastore | Invoke-AttachDatastore
.EXAMPLE Get-Datastore Datastore | Get-DatastoreMountInfo | Sort Datastore, VMHost | FT -AutoSize
#>
...
Function Get-DatastoreMountInfo {
	...
	End {
		...
		#Build the array of Datastore Objects
		if (-not $Datastore) {
			$allDatastores = get-cluster $Cluster | Get-Datastore -Server $vserver
		}
		$allDatastores = $allDatastores | ? {$_.pstypenames -contains "VMware.VimAutomation.ViCore.Impl.V1.DatastoreManagement.DatastoreImpl"}
		if (-not $allDatastores){
			Throw "No Datastores found.`nIs ""$Datastore"" a Datastore Object?"
		...
		foreach ($dsHost in $allHosts){
			...
			foreach ($device in $devices){
				if ($allDatastoreNAAs -contains $device.canonicalName){
					if (($device.LocalDisk -eq $false) -and ($device.vendor -eq $vendor)){
						...
						$Info = "" | Select Datastore, VMHost, Lun, Mounted, State, NAA, Path, DisplayName
						$Info.VMHost = $dsHost.hostview.name
						$Info.Datastore = $thisDatastore.name
						$Info.Lun = $device.canonicalName
						$Info.NAA = $($device.canonicalName.split("naa.")[1])
						$Info.Path = $device.DeviceName
